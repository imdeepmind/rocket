...
// ---------------------------------------------------------------------------
// Helper: create a Fastify instance with the delete-me route wired up
// ---------------------------------------------------------------------------
...
describe('DELETE /auth/user/me', () => {
  ...
  describe('guard conditions', () => {
    test('should NOT register the route when enabled is false', async () => {
      ...
      const app = await createDeleteMeApp(authentication);
    ...
    test('should NOT register the route when model is not found in models', async () => {
      const app = await createDeleteMeApp(upAuthConfig, {});
  ...
  describe('happy path', () => {
    test('should return 200 and delete the user when user exists', async () => {
      const app = await createDeleteMeApp(upAuthConfig);

      const token = app.jwt.sign({id: 1, email: 'alice@example.com'});

      // Mock transaction: BEGIN, SELECT (user exists), DELETE, COMMIT
      pgClientQueryMock
      ...
      expect(body.message).toBe('User profile deleted successfully');
